...
public:
    ...
    // The registered objects are not deleted on shutdown; they would have to do that themselves within 
    // OnSystemDestroy or via a compiler-generated static destruction.
    void RegisterDestroyCallback();
    void PushDestroyCallbacks() { RegisterDestroyCallback(); } // For backward compatibility.

protected:
    SystemSingletonInternal() :
        NextShutdownSingleton(nullptr)
    {
    }
    ...
    // This function is useful for terminating threads at the right time before the rest of the system is shut down.
    // Note: The singleton must not delete itself here, as OnSystemDestroy will subsequently be called for it.
